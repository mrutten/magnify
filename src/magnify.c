...
void put_image(void) 
{
    ...
    XImage *img = XGetImage(display, root, lx, ly, (unsigned int)capture, (unsigned int)capture, AllPlanes, ZPixmap);

    for (int sy = 0; sy < capture; sy++) {
        for (int sx = 0; sx < capture; sx++) {
            uint32_t i = (unsigned int)(sy * capture + sx) * 4;

            uint32_t base_px = (unsigned int)(sy * box_size * ratio) + (unsigned int)(sx * ratio);
            for (uint8_t ry = 0; ry < ratio; ry++) {
                uint32_t bn = (base_px + (ry * box_size)) * 4;
                for (uint8_t rx = 0; rx < ratio; rx++) {
                    image_data[bn + 0] = (unsigned char)img->data[i + 0];
                    image_data[bn + 1] = (unsigned char)img->data[i + 1];
                    image_data[bn + 2] = (unsigned char)img->data[i + 2];
                    bn += 4;
                }
            }
        }
    ...
    XImage *img_2 = XCreateImage(display, visual, (unsigned int)depth, ZPixmap, 0, (char *)image_data, box_size, box_size, 32, 0);

    XPutImage(display, window, gc, img_2, 0, 0, padding, padding, box_size, box_size);
...
int main(void)
{
    ...
    window = XCreateSimpleWindow(display, root, display_width - (total_box_size + 10),
            display_height - (total_box_size + 10), box_size + padding, box_size + padding, 0, 0,
            0x000000);
    ...
    XSelectInput(display, window, KeyPressMask | ExposureMask | PointerMotionMask | ButtonPressMask);
    XFlush(display);
    ...
    if (grab_result != GrabSuccess) { 
        fprintf(stderr, "Warning: Failed to grab pointer (%d). Continuing anyway.\n", grab_result);
            ...
            switch (ev.type) {
                ...
                case KeyRelease:
                    switch (ev.xkey.keycode) {
                        ...
                        case 24: // q
                            running = false;
                            break;
                        case 46: case 114: x++; put_image(); break; // Right
                        case 43: case 113: x--; put_image(); break; // Left
                        case 45: case 111: y--; put_image(); break; // Up
                        case 44: case 116: y++; put_image(); break; // Down
                    }
                    break;
                ...
                case ButtonPress:
                    ...
                    if (ev.xbutton.button == 4) { // scroll up
                        ...
                        capture = box_size / ratio;
                        put_image();
                    } else if (ev.xbutton.button == 3) { // right click center
                        int32_t mx = ev.xbutton.x_root - box_size / 2;
                        int32_t my = ev.xbutton.y_root - box_size / 2;
                        if (mx < 10) mx = 10;
                        if (my < 10) my = 10;
                        if (mx > display_width - box_size - 10) mx = display_width - box_size - 10;
                        if (my > display_height - box_size - 10) my = display_height - box_size - 10;
