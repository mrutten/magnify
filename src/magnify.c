...
void put_image(void) {
    ...
    for (int sy = 0; sy < capture; sy++) {
        for (int sx = 0; sx < capture; sx++) {
            ...
            uint32_t base_px = (unsigned int)(sy * box_size * ratio) +
    ...
    XImage *img_2 =
        XCreateImage(display, visual, (unsigned int)depth, ZPixmap, 0,
                     (char *)image_data, box_size, box_size, 32, 0);
...
int main(void) {
    ...
    window = XCreateSimpleWindow(
        display, root, display_width - (total_box_size + 10),
        display_height - (total_box_size + 10), box_size + padding,
        box_size + padding, 0, 0, 0x000000);
    ...
    while (running) {
        ...
        XQueryPointer(display, root, &root, &child, &root_x, &root_y, &win_x,
                      &win_y, &mask);
            ...
            case ButtonPress:
                ...
                if (ev.xbutton.button == 4) { // scroll up
                    ...
                    if (mx > display_width - box_size - 10)
                        mx = display_width - box_size - 10;
                    if (my > display_height - box_size - 10)
                        my = display_height - box_size - 10;
